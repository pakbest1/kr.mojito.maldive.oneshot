/*
	...
	1. gfnPlus 						: 소수점 이하 더하기 연산 처리가 부적합하게 처리되는 경우 정정처리 하기 위한 함수
	2. gfnDivide 					: 소수점 이하 나누기 연산 처리가 부적합하게 처리되는 경우 정정처리 하기 위한 함수
	3. gfnMultiply 					: 소수점 이하 곱하기 연산 처리가 부적합하게 처리되는 경우 정정처리 하기 위한 함수
	4. gfnIntToHanGul 				: 숫자를 한글로 표기
	5. gfnIntToHanJa 				: 숫자를 한자로 표기
	6. gfnBytesToSize 				: 파일 사이즈 환산
	7. gfnEvenRound 				: 반올림할 자리의 값이 1~4인 경우는 버리고, 6~9인 경우는 올림 처리한다
	8. gfnGetMaskFormatNumToString 	: 주어진 숫자 형식값에 Mask Format 처리된 문자열을 반환합니다
*/

//넥사크로에 정의된 Form의 Prototype 객체를 변수로 가져오기
var _pForm = nexacro.Form.prototype;

_pForm.point = ".";					//소수점 표시자 (locale에 따라 정의)
_pForm.separator = ",";				//숫자 단위 구분 표시자 (locale에 따라 정의)
_pForm.separatorGrouping = "\\3";		//통화 표시에서 separator 규칙 정의 (locale에 따라 정의)
_pForm._numberMaskCache = {};			//mask format cache
...
* @memberOf this
*/
_pForm._adjustFloat = function(n)
{
...
* @memberOf this
*/
_pForm._getMaxPrecision = function(values) 
{
	var maxPrecision = 0,
		currentPrecision,
		tempValue;
	...
	{
		//trace(arg[i]);
		tempValue = this._adjustFloat(values[i]);
...
* 따라서 this.gfnPlus 함수를 사용하여 처리합니다.
* @param {...number} arguments . 예: (1660, -1559.9, 0.33) 
* @return {number} 결과값 반환
* @example
* var a = 0.1,
*     b = 0.2;
* trace(this.gfnPlus(a, b)); // output : 0.3
* trace(1660 - 1559.9 + 0.33); // output : 100.42999999999991
* trace(this.gfnPlus(1660, -1559.9, 0.33)); // output : 100.43
* @memberOf this
*/
_pForm.gfnPlus = function() 
{
	var maxPrecision = 0,
		currentPrecision, 
		total = 0,
	...
	maxPrecision = this._getMaxPrecision(arguments);
 ...
 * 왜 필요하나? -> [plus 함수 설명 내용 참조]{@link this.gfnPlus}<br><br>
 ...
 * trace(this.gfnDivide(1660, -1559.9, 0.33)); // output : -3.22475993993399
 * @memberOf this
 */ 
_pForm.gfnDivide = function() 
{
	var maxPrecision = 0;
	var total = 0;	
	...
	do{
		...
		i++;
		maxPrecision = Math.pow(10, (this._getMaxPrecision(tempValues) - 1));
		//정수로 만들고, 나누기 처리.
		for(var j=0; j<2; j++){
			tempValues[j] *= maxPrecision;
		}
		
		total = this._adjustFloat(tempValues[0]/tempValues[1]);
 ...
 * trace(this.gfnMultiply(1660, -1559.9, 0.33)); // output : -854513.22
 * @memberOf this
 */ 
_pForm.gfnMultiply = function() 
{
	var maxPrecision = 0;
	var total = 0;	
	var count = arguments.length -1;	
	...
	do{
		...
		resultPrecision = this._getMaxPrecision(tempValues);
		...
		//정수로 만들고, 곱하기 처리.
		for(var j=0; j<2; j++){
			tempValues[j] = this._adjustFloat(tempValues[j] * maxPrecision);
		}
 ...
 * @memberOf this
 */ 	 
_pForm.gfnIntToHanGul = function(val)
{
 ...
 * @example
 * var val = 1200340500.01;
 * var str = this.gfnIntToHanJa(val);
 ...
 * @memberOf this
 */ 	 
_pForm.gfnIntToHanJa = function(val)
{
	if ( !this.gfnIsString(val) )
	{
		val = String(val);
...
*     precision = 2,
*     unit = false;
* var result = this.gfnBytesToSize(size, precision, unit);
* trace(result); // output : 976.00
* result = this.gfnBytesToSize(size, 0);
* trace(result); // output : 976 MB
* result = this.gfnBytesToSize(size);
* trace(result); // output : 976.0 MB
* @memberOf this
*/	
_pForm.gfnBytesToSize = function(bytes, precision, unit) 
{
...
* @param {number} decimalPlaces 소수부.
* @return {number} 결과값
* @example
* alert(this.gfnEvenRound(1.5)); // 2
* alert(this.gfnEvenRound(2.5)); // 2
* alert(this.gfnEvenRound(1.535, 2)); // 1.54
* alert(this.gfnEvenRound(1.525, 2)); // 1.52
* @memberOf this
*/		
_pForm.gfnEvenRound = function(num, decimalPlaces)
{
 ...
 * @memberOf this
 */	
_pForm._parseNumberMask = function(strMask)
{
	var res = this._numberMaskCache[strMask];
	if ( res ) return res;
	...
	this._numberMaskCache[strMask] = res;
	return res;
}

/**
 * 숫자형 문자에서 부호, 소수점, 정수부, 소수부 추출.
 * @private
 * @param {string} strText 숫자형 문자.
 * @return {array} 추출 정보
 * @memberOf this
 */	
_pForm._numberSplit = function(strText) 
{
	// 부호 분리.
	var nBegin = 0, nSign;
	...
	var nPoint = strText.indexOf( this.point ,nBegin ),
		strNumber = "", bPoint, strDecimal = "";
	if( nPoint < 0 ) 
	{
 ...
 * @memberOf this
 */	
_pForm._removeMask = function(str) 
{
	str = str.trim();
	var ntxtLen = str.length,
		i,
		pointVal = this.point,
		separatorVal = this.separator;
 ...
 * @memberOf this
 */
_pForm._applyComma = function(strNumber) 
{
	var grouping = this.separatorGrouping;
	var thousands_sep = this.separator;
 ...
 * @memberOf this
 */	
_pForm._normalizeValue = function(strValue, bTrim) 
{
	...
	// 정규형식으로 만듬.
	if( strValue.charAt( strValue.length -1 ) == this.point )
		strValue = strValue + "0";
 ...
 * @example
 * var val = 1234.567;
 * var result = this.gfnGetMaskFormatNumToString(val, "99.99");
 * trace(result); // output : 1234.56
 * result = this.gfnGetMaskFormatNumToString(val, "9900.0099");
 * trace(result); // output : 1234.567
 * result = this.gfnGetMaskFormatNumToString(val, "9,999.9");
 * trace(result); // output : 1,234.5
 *
 * val = 1.2;
 * result = this.gfnGetMaskFormatNumToString(val, "99.99");
 ...
 * @memberOf this
 */		
_pForm.gfnGetMaskFormatNumToString = function(value, strMask)
{
	var strText = value + "";
