...
pForm.gfnSetMain = function(bInit)
{
	...
	nexacro.MdiWorkVFrameSet00.set_separatesize("40,*,0");
	nexacro.WorkFrame.set_separatesize("42,*,0");

 	if (bInit == false) {
 		//objApp.gvFrmMdi.form.fnSetStyle("main");
		nexacro.MdiFrame.form.fnSetStyle("main");
 	}
	
	if (this.gfnIsNull(nexacro.MainFrame.formurl)) {
		//objApp.gvFrmMain.set_formurl(objApp.MAIN_FORM_PATH);
		nexacro.MainFrame.set_formurl(nexacro.MAIN_FORM_PATH);
	}
	
	//objApp.gvFrmMain.form.setFocus();
	nexacro.MainFrame.form.setFocus();
};
...
pForm.gfnShowLeftFrame = function () {
	...
	*/
	nexacro.HFrameSet00.set_separatesize('270,*');  // 210
	nexacro.TopFrame.form.btnLeftCntlr.set_cssclass("btn_FT_menuClose");  // btn_MDI_Close
	nexacro.LeftFrame.form.fvMenuStatus = "open";
};
...
pForm.gfnHideLeftFrame = function () {
	...
	*/
	...
	nexacro.TopFrame.form.btnLeftCntlr.set_cssclass("btn_FT_menuOpen");
	nexacro.LeftFrame.form.fvMenuStatus = "close";
};
...
pForm.gfnFormOnload = function (objForm)
{
	...
	if (objForm.opener)
	{
		if (objForm.parent instanceof nexacro.ChildFrame)
		{
			...
			// WMS Project #1
			let tOwnerFrame = this.getOwnerFrame();
			if (tOwnerFrame) {
				tOwnerFrame.set_border('0px');  // 팝업 Border 삭제
			}
			// /WMS Project #1
		}
	}
	...
	if (this.fnPopupFormOnload) { this.fnPopupFormOnload(); }
	// /WMS Project #2
};
...
pForm.gfnInitComp = function(objForm)
{
	...
	for (var i=0; i<nLength; i++) {
		var nxComp = arrComp[i];
		
		if (nxComp instanceof nexacro.Div) {
			//URL로 링크된 경우에는 존재하는 경우에는 해당 링크된 Form Onload에서 처리하도록 한다.
			if (this.gfnIsNull(nxComp.url)) this.gfnInitComp(nxComp.form);
		} 
		else if (nxComp instanceof nexacro.Tab) {
			var nPages = nxComp.tabpages.length;
			
			for (var j=0; j<nPages; j++) {	
				//URL로 링크된 경우에는 존재하는 경우에는 해당 링크된 Form Onload에서 처리하도록 한다.
				if (this.gfnIsNull(nxComp.tabpages[j].url)) this.gfnInitComp(nxComp.tabpages[j].form);
			}
		} 
		else {
			// WMS Grid 기능
			if (nxComp instanceof nexacro.Grid) {  // Grid 기능
				this.initGridComponent(nxComp);  // this.gfnSetGrid(nxComp);
			} else
			
			// WMS Splitter 기능
			if (nxComp instanceof nexacro.Static && nxComp.uDirection) { // Splitter 기능
			...
			// WMS Clipboard
			this.initClipboardComponent(nxComp);
...
{	
	...
	if (e.ctrlkey && e.altkey && e.keycode == 68) 
	{
		...
		this.gfnOpenPopup("popupDebug", "common::Debug.xfdl", objArg, "", objOption);
