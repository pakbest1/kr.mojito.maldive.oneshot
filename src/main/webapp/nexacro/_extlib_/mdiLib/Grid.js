...
pForm.GridConfig = {
	...
	sort : {
		...
		ascText  			: "  ▲",   // " <fs v='10'>▲</fs>"  ,	// ascending text
		descText 			: "  ▼",   // " <fs v='10'>▼</fs>"  ,	// descending text
		
		ascDecoText         : "  <fs v='10'>▲</fs>",  // ascending  decorate text - cell displayType = decorateText
		desDecocText	    : "  <fs v='10'>▼</fs>",  // descending decorate text - cell displayType = decorateText
...
*/
pForm.gfnGridOnheadclick = function(objGrid, e)
{
	if (this.gfnIsGridFilterCell  (objGrid, e)) return;  // 필터 Cell 이면 정렬처리 제외
	if (this.gfnIsGridCheckboxCell(objGrid, e)) return;  // 체크박스인 경우
	
	// sort
	trace('gfnGridOnheadclick - sort');
...
pForm.gfnSetGrid = function(objGrid)
{
	...
	var arrProp = this._getGridUserProperty(objGrid);  // Grid의 UserProperty 설정
	
	wms.initGridComponent(objGrid, this);  // [2023.12.21] sg.park - WMS_Grid.js 에서 그리드 설정 처리예정
	
	//if(objApp.gvUseGridContextMenu != true) return;
	if (this.gfnIsNull(arrProp)) return;               // 설정할 속성이 없을땐 리턴
	objGrid.arrprop = arrProp;
	
	// [2023.12.21] sg.park - Sort 설정은 - WMS_Grid.js > initGrid() 함수에서 진행예정
	//if (arrProp.indexOf('sort')>=0) { objGrid.uFun_sort = true; }
	//objGrid.addEventHandler('onheadclick', this.gfnGridOnheadclick, this);
...
* @param {Evnet}  e	   - 헤드클릭이벤트
* @return  N/A
* @example objGrid.gfnIsGridCheckboxCell(obj, e);
*/
pForm.gfnIsGridCheckboxCell = function(objGrid, e)
{
	if (objGrid.readonly == true) return true;
	var nCell = e.cell, nCol = e.col;
	
// 	if (objGrid.getCellProperty("head", nCell, "edittype") != "checkbox") return false;
// 	if (objGrid.getCellProperty("body", nCell, "edittype") != "checkbox") return false;
	if (objGrid.getCellPropertyValue(e.row, e.cell, 'edittype') != 'checkbox') { return false; }
...
* @example this.gfnGridGetBodyCellIndex(this.grid, 0);
*/
