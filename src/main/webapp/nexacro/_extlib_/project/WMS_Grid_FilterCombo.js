...
pForm.Grid_FilterCombo_Show = function(grid, sFiltertype) {
	...
	grid.config = Object.assign(grid.config||{}, {
		type   : 'combo',
		css    : 'filter',
		editid : 'edtFilter',
		popupid: 'popFilter',
	...
	let form = grid._getForm()||(grid.getBindDataset()||grid).parent, config = grid.config;
	...
	let nHeadRow, nHeadCol, sText, sBindColumn, nHeadCellCnt = grid.getCellCount('head');  // Head의 Cell 갯수 가져오기
	for (let i=0; i<nHeadCellCnt; i++) {
		nHeadRow = grid.getCellProperty('head', i, 'row');
		grid.setCellProperty('head', i, 'cssclass'   , 'fltr');  // fltr 행 인식자
		...
		if (nHeadRow == filterHeadRowIndex) {
			...
			sText = grid.getCellProperty('body', nHcol, 'text');
			...
			if (sText && sText.indexOf('bind:') > -1) {
				...
				grid.setCellProperty('head', i, 'cssclass'   , 'fltr filter');  // [2023.12.01] sgpark - Filter Icon 표시
...
pForm.Grid_FilterCombo_ReMakeCells = function(grid) {
	...
	let i, j, nHeadRow, nHeadCol, sText, sBindColId, sBindColKw, sBindColVl;
	...
	for (i=0; i<headCellCunt; i++) {
		...
		if (nHeadRow == filterHeadRowIndex) {
			...
			sText    = grid.getCellProperty('body', nHeadCol, 'text');
			
			// Bind된 Cell일 경우
			if (sText && sText.indexOf("bind:") > -1) {
				// bind정보 찾기
				sBindColId = sText.replace('bind:', '');
...
pForm.Grid_FilterCombo_GetFilterValue = function(objPopDiv) {
	...
	let iRowCunt = fds.getRowCount(), iRowCuntNF = fds.getRowCountNF(), fKeyword = '', fValue = '';
	...
	fpopform.opcomps.configcell.value   = fValue  ;
...
pForm.Grid_FilterCombo_Apply = function(grid) {
	...
	for (let i=0; i<cellsCunt; i++) {
		if (cells[i] && cells[i].bind) {  // Cell 설정정보가 존재하고 bind 정보가 있을 경우
			fcolId = cells[i].bind;
			...
			if (fcolVl) {  // fcolVl &&
				...
				fstring += '('
				acolVl = (fcolVl||'').replace(/\{empty\}/g, '').split(',');
				acolVlCunt = acolVl.length;

// [2024.02.01] sgpark - 당분간 Like 검색은 봉인
// 				if (filtertype=='like') {
...
pForm.Grid_FilterCombo_OnHeadClick = function(obj, e) {  // obj:nexacro.Grid,e:nexacro.GridClickEventInfo
	...
	let fedit, oRect, nLeft, nTop, nWidth, nHeight, sText
...
pForm.Grid_FilterCombo_Edit_OnKeyDown = function(obj, e) {  // obj:nexacro.Edit,e:nexacro.KeyEventInfo
	...
	form.Grid_FilterCombo_Apply(objGrid);                      // 필터 실행 함수 호출
};
