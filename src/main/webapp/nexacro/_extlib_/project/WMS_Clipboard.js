...
pForm.nxCopyPaste_OnKeydown = function(obj, e) {
	...
	if (e.keycode == 88) {  // Ctrl+X
		this.nxCopyPaste_ActionCut  ( efcomp );  // console.log('Ctrl + X : Cut   - '+ efcomp.name);
	}
...
pForm._enClipboard = function(grid) {
	...
	if ([ 'multirow', 'multiarea' ].includes( selecttype )) {
		this.alert(errMesg);
		return;
	}
	
	if (grid.selectstartrow.length > 1) { // 멀티블록 미지정
		this.alert(errMesg);
		return;
	}
	
	// ['cell', 'area'].includes( seletype )
	let startrow  = nexacro.toNumber(grid.selectstartrow[0]);
	let endrow    = nexacro.toNumber(grid.selectendrow  [0]);
	let startcell = this.getCellIdx(grid, 'body', grid.selectstartcol, grid.selectstartsubrow)[0];
	let endcell   = this.getCellIdx(grid, 'body', grid.selectendcol  , grid.selectendsubrow  )[0];
	...
	let selectText = grid.getEditSelectedText();
...
pForm._deClipboard = function(clip, grid) {
	if (!clip || !grid || grid.readonly || !grid.getBindDataset()) { return; }
